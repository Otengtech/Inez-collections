    ...
    <div className="bg-white rounded-xl p-5 shadow-sm hover:shadow-md transition-all duration-300 border border-gray-100">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h4 className="font-semibold text-black text-sm">{review.guestName}</h4>
              {review.isVerified && (
            ...
            {isAuthor && (
              <>
                <button
                  onClick={handleEdit}
                  disabled={loading}
                  className="w-8 h-8 rounded-lg hover:bg-gold-50 text-gray-400 hover:text-gold-600 transition-colors flex items-center justify-center"
                  title="Edit review"
                >
        ...
        <p className="text-black/70 text-sm leading-relaxed">
          ...
          {isLongComment && (
            <button
              ...
              className="text-gold-600 hover:text-gold-700 text-sm font-medium ml-1"
            >
      ...
      <div className="flex items-center justify-between pt-3 border-t border-gray-100">
        <div className="flex items-center gap-4">
          <button
            ...
            className={`flex items-center gap-1.5 text-sm transition-colors ${
              isHelpful 
                ? 'text-gold-600' 
                : 'text-black/40 hover:text-gold-600'
            } disabled:opacity-50`}
          >
            <FontAwesomeIcon icon={isHelpful ? faThumbsUp : faThumbsUpRegular} />
            <span>{helpfulCount}</span>
            <span className="text-xs hidden sm:inline">helpful</span>
          </button>
          
          <button className="text-black/40 hover:text-red-500 transition-colors text-sm flex items-center gap-1">
        ...
        {review.updatedAt && review.updatedAt !== review.createdAt && (
          <span className="text-[10px] text-black/30">(edited)</span>
        )}
      </div>
    </div>
  )
