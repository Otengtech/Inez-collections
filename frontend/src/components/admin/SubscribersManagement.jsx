    ...
    <div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-black/40 mt-1">Manage your newsletter subscribers</p>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={fetchSubscribers}
            className="inline-flex items-center justify-center gap-2 bg-[#F4F6F2] text-black/70 font-medium px-4 py-3 rounded-xl hover:bg-gray-200 transition-all duration-300"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-xl p-6 shadow-sm hover:shadow-md transition-all">
          <p className="text-sm text-black/50">Total Subscribers</p>
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm hover:shadow-md transition-all">
          <p className="text-sm text-black/50">Inactive</p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-[#F4F6F2]">
                <tr>
                  <th className="px-6 py-4 text-left text-xs font-semibold text-black/50 uppercase tracking-wider">Email</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold text-black/50 uppercase tracking-wider">Status</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold text-black/50 uppercase tracking-wider">Subscribed</th>
                ...
                {filteredSubscribers.length > 0 ? (
                  filteredSubscribers.map((sub) => (
                    <tr key={sub._id} className="hover:bg-[#F4F6F2]/50 transition-colors">
                      <td className="px-6 py-4 text-sm text-black">{sub.email}</td>
                        ...
                        <div className="flex items-center gap-2">
                          {sub.isActive ? (
                            ...
                            <button
                              ...
                              className="w-9 h-9 rounded-lg bg-green-50 text-green-500 hover:bg-green-100 transition-colors flex items-center justify-center"
                              title="Resubscribe"
                            >
          ...
          <div className="bg-white rounded-2xl p-6 md:p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-sm text-black/40 mt-1">Send an email to all active subscribers</p>
              </div>
              <button
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows="10"
                  className="w-full px-4 py-3 bg-[#F4F6F2] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#D6F04C]/40 focus:bg-white transition-all duration-300 text-sm border border-transparent focus:border-[#D6F04C] placeholder:text-black/30 font-mono"
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-gray-100">
                <button
                  ...
                  disabled={sending}
                  className="flex-1 inline-flex items-center justify-center gap-2 bg-black text-white font-semibold px-6 py-3.5 rounded-xl hover:bg-black-800 transition-all duration-300 shadow-lg hover:shadow-xl disabled:opacity-50"
                >
                  {sending ? (
                    <>
