...
const ProductCard = ({ product }) => {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      ...
      try {
        ...
        // If API fails, use product data as fallback
    ...
    fetchReviews()
  }, [product._id, product.rating, product.numReviews])
  ...
  // Use review stats from API or fallback to product data
  const displayRating = reviewStats.averageRating || product.rating || 0
  const displayReviewCount = reviewStats.totalReviews || product.numReviews || product.ratings?.length || 0
    ...
    >
      ...
      <div className="relative aspect-square overflow-hidden bg-gray-100">
        ...
        <button
          ...
          className={`absolute bottom-3 right-3 w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center shadow-lg transform transition-all duration-300 z-10 ${
        ...
        >
          <span className="bg-white/90 backdrop-blur-sm text-black px-4 py-2 rounded-full text-sm font-medium flex items-center gap-2 hover:bg-white transition-colors">
        ...
        {isHovered && (
          <div className="mt-2 pt-2 border-t border-gray-100 animate-fade-in">
            <div className="flex items-center justify-between text-[8px] md:text-[10px] text-gray-400">
              ...
              <span className="flex items-center gap-1">
                <FontAwesomeIcon icon={faClock} className="text-[#D6F04C]" />
                2-5 days
              </span>
              <span className="flex items-center gap-1">
                <FontAwesomeIcon icon={faTag} className="text-[#D6F04C]" />
                {product.category}
              </span>
            </div>
          </div>
